...
@section('page-action')
    <div>
        ...
        <a href="{{ route('projectbug.list', [$project->id]) }}" class="btn btn-sm btn-primary"
            data-bs-toggle="tooltip"title="{{ __('List View') }}">
            <i class="ti ti-list text-white"></i>
        </a>
        @permission('bug create')
            <a class="btn btn-sm btn-primary" data-ajax-popup="true" data-size="lg" data-title="{{ __('Create New Bug') }}"
                data-url="{{ route('projects.bug.report.create', $project->id) }}" data-toggle="tooltip"
                title="{{ __('Add Bug') }}"><i class="ti ti-plus"></i></a>
        @endpermission
        <a href="{{ route('projects.show', [$project->id]) }}" class="btn-submit btn btn-sm btn-primary "
            data-toggle="tooltip" title="{{ __('Back') }}"><i class="ti ti-arrow-back-up"></i></a>
                                    ...
                                    @foreach ($stage->bugs as $bug)
                                        <div class="card" id="{{ $bug->id }}">
                                            <div class="position-absolute top-0 start-0 pt-3 ps-3">
                                                @if ($bug->priority == 'Low')
                                                    <div class="badge bg-success p-2 px-3 rounded"> {{ $bug->priority }}
                                                    </div>
                                                @elseif($bug->priority == 'Medium')
                                                    <div class="badge bg-warning p-2 px-3 rounded"> {{ $bug->priority }}
                                                    </div>
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end">
                                                            @permission('bug show')
                                                                <a class="dropdown-item" data-ajax-popup="true" data-size="lg"
                                                                    ...
                                                                    data-url="{{ route('projects.bug.report.show', [$bug->project_id, $bug->id]) }}"><i
                                                                        class="ti ti-eye"></i>
                                                                    {{ __('View') }}</a>
                                                            @endpermission
                                                            @permission('bug edit')
                                                                <a class="dropdown-item" data-ajax-popup="true" data-size="lg"
                                                                    ...
                                                                    data-url="{{ route('projects.bug.report.edit', [$bug->project_id, $bug->id]) }}"><i
                                                                        class="ti ti-pencil"></i>
                                                                    {{ __('Edit') }}</a>
                                                            @endpermission
                                                            @permission('bug delete')
                                                                <form id="delete-form-{{ $bug->id }}"
                                                                    action="{{ route('projects.bug.report.destroy', [$bug->project_id, $bug->id]) }}"
                                                                    method="POST">
                                                                    @csrf
                                                                    @method('DELETE')
                                                                    <a href="#"
